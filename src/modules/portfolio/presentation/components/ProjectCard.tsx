...
export function ProjectCard({ project }: ProjectCardProps) {
  return (
    <article className="group overflow-hidden rounded-3xl border border-[var(--color-border)] bg-[var(--color-surface)]/50 transition hover:-translate-y-1 hover:shadow-[0_18px_50px_rgba(0,0,0,0.2)]">
      <Link href={`/projetos/${project.slug}`}>
        ...
        <div className="space-y-4 p-6">
          <h3 className="text-2xl font-semibold text-[var(--color-text)]">{project.title}</h3>
